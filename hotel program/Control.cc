...
Control::Control(){
  ...
  hotel = new Hotel("HOTEL", resMgr);
  resMgr->setHotel(hotel);
  stayrec = new StayRecorder("stay");
  guestrec = new GuestRecorder("guest");
  uprec = new UpgradeRecorder("upgrade");

  resMgr->subscribe(stayrec);
  resMgr->subscribe(guestrec);
  resMgr->subscribe(uprec);
